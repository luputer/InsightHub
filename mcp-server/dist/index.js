...
import { Keypair, Contract, TransactionBuilder, Networks, nativeToScVal, scValToNative, BASE_FEE, rpc, } from '@stellar/stellar-sdk';
const CONTRACT_ID = 'CBN6DV6AOZWNROEXKYWJGZLV2LODUE4LIQTVDJWYXDUH6K7X2XHIW5RL';
...
function toScVal(val) {
    const type = typeof val === 'string' && (val.startsWith('G') || val.startsWith('C'))
        ? 'address'
        : undefined;
...
async function invoke(method, args) {
    ...
    if (pollResult.status === 'FAILED') {
        throw new Error(`Transaction failed: ${pollResult.resultXdr?.result().toString()}`);
    }
    if (pollResult.status !== 'SUCCESS') {
        throw new Error(`Transaction not found: ${sendResult.hash}`);
    }
    const retval = pollResult.returnValue;
    if (retval)
...
const mcpServer = new Server({ name: 'marketplace-mcp', version: '1.0.0' }, { capabilities: { tools: {} } });
...
mcpServer.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            ...
            case 'get_listing': {
                const result = await simulate('get_listing', [BigInt(a.id)]);
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            case 'get_my_listings': {
                const result = await simulate('get_my_listings', [a.caller]);
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            case 'get_orders': {
                const result = await simulate('get_orders', [BigInt(a.listing_id)]);
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            case 'list_item': {
            ...
            case 'buy_item': {
                await invoke('buy_item', [sellerAddress, BigInt(a.id)]);
                return { content: [{ type: 'text', text: `Successfully purchased listing ${a.id}` }] };
            }
            case 'cancel_item': {
                await invoke('cancel_item', [sellerAddress, BigInt(a.id)]);
